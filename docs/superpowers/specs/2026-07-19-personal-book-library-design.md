...
Prisma 7 keeps `DATABASE_URL` and the seed command in `backend/prisma.config.ts`; `schema.prisma` declares only the SQLite provider. Its `prisma-client` generator writes an ignored client to `backend/generated/`, and the runtime client uses `@prisma/adapter-better-sqlite3`. The tracked initial SQL migration is generated with `prisma migrate diff`; `npm run prisma:setup --workspace backend` creates the local SQLite file and uses schema-driven `prisma db push` with a portable absolute path.
...
- Final quality gates: `npm test`, `npm run typecheck`, `npm run build`, tracked Prisma migration SQL plus `npm run prisma:setup --workspace backend` from a clean database, และตรวจ checklist ในโจทย์ด้วย browser

## 10. Delivery and documentation

`README.md` ต้องมี prerequisites, install, environment variables, tracked migration SQL plus `npm run prisma:setup --workspace backend` and seed commands, run commands, test commands, URLs และ test username/password `REFLECTION.md` ต้องใช้ภาษาของเจ้าของงานและไม่เกิน 10 บรรทัด `.env.example` ใส่ค่า local demo ที่ไม่ใช่ production secret ส่วน `.env`, `*.db`, coverage และ build output ถูก ignore
